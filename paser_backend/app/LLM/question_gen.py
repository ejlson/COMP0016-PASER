...
class LLMQuestionGenerator(BaseQuestionGenerator):
    ...
    def generate(
        ...
        try:
            ...
            The above output has a formatting error, output the same text without JSON format errors by adding either missing symbols, keys or values. Your output should contain NOTHING else but the corrected output, common mistakes include missing key '"tool_name"', missing key '"sub_question"', missing ',' missing '{{', unnessasary space in key' '.
            corrected output:
"""
            prediction = self._llm.complete(prompt)
            parse = self._prompt.output_parser.parse(str(prediction))
...
"""
            ...
            prediction = self._llm.chat(prompt)
            parse = self._prompt.output_parser.parse(prediction)
